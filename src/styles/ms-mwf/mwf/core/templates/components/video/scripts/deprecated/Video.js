import { addClass, removeClass, addEventListeners, getClientRect } from '../../../deprecated/helpers.js';
import Slider from '../../slider/scripts/deprecated/Slider.js';
...
Video.prototype.init = function() {
    ...
    this.timeCurrent = this.element.querySelector('.f-current-time');
    this.timeDuration = this.element.querySelector('.f-duration');
    ...
    //Checking if time has aria-label
    if(!this.timeCurrent.hasAttribute("aria-label")) {
        console.log(this.timeCurrent.hasAttribute("aria-label"));
        return;
    }
    ...
    if (this.videoObj[0].getAttribute('autoplay') !== null)this.playPause('autoplay');
...
Video.prototype.videoMethods = function(e) {
    //Update time
    this.timeCurrent.childNodes[0].nodeValue = this.timeFormat(this.videoObj[0].currentTime.toFixed(0));
    this.timeDuration.innerHTML = this.timeFormat(this.videoObj[0].duration.toFixed(0));
    ...
    var playBtn = this.playButton;
    var playTool = this.playTooltip.innerHTML;

    this.videoObj[0].onended = function(e) {
        removeClass(playBtn, 'glyph-pause');
        playBtn.setAttribute('aria-label', "Play");
        addClass(playBtn, 'glyph-play');
        playTool = "Play";
    };
