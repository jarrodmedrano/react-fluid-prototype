...
CompareChart.prototype.getHeights = function() {
    ...
    for (var i = 0; i < this.columns.length; i++) {
        this.rowEls = this.columns[i].querySelectorAll('.f-row');
        var heights = [];

        for (var j = 0; j < this.rowEls.length; j++) {
            this.rowEls[j].style.height = 'auto';
            heights.push({
                element: this.rowEls[j],
                height: getClientRect(this.rowEls[j]).height
            });
        }
...
CompareChart.prototype.equalizeHeights = function() {
    for (var l = 0; l < this.rowEls.length; l++) {
        var tempHeight = null;
        var tempEl = [];
