import _ from 'lodash';

export const headingBlock = {
    "heading": "-- Hero Heading --",
    "subheading": "-- Subheading --",
    "paragraph": "-- Paragraph -- lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis vitae convallis eros, nec iaculis nisi. Duis imperdiet elementum diam, eu tempor quam tempus sit amet. Suspendisse eu tortor quis odio faucibus tempus sed",
    "button": {
        "layout": "button",
        "link": "#/windows",
        "text": "Call to action",
        "ariaLabel": "ariaLabel",
        "textColor": "white",
        "backgroundColor": "#0078D7"
    }
};

export const pictureBlock = {
    "altText": "AltText",
    "ariaLabel": "AriaLabel",
    "pictures": [
        {
            "src": "http://placehold.it/2400x1350/2F2F2F/171717",
            "minWidth": "1779"
        },
        {
            "src": "http://placehold.it/1778x1000/2F2F2F/171717",
            "minWidth": "1400"
        },
        {
            "src": "http://placehold.it/1259x472/2F2F2F/171717",
            "minWidth": "1084"
        },
        {
            "src": "http://placehold.it/1083x609/2F2F2F/171717",
            "minWidth": "768"
        },
        {
            "src": "http://placehold.it/767x431/2F2F2F/171717",
            "minWidth": "540"
        },
        {
            "src": "http://placehold.it/539x303/2F2F2F/171717",
            "minWidth": "0"
        }
    ]
};

export const videoBlock = {
    "src": "http://clips.vorwaerts-gmbh.de/big_buck_bunny.mp4"
};

export const ratings = {
    "rating": "3",
    "bestRating": "5",
    "ratingText": "See reviews",
    "reviewLink": "https://www.microsoftstore.com/store/msusa/en_US/pdp/Surface-Book/productID.325716000#ratingsandreviews"
};

import logo from '../../src/img/vertical/ms-logo.png';

export const Compare = {
    "type": "compare",
    "headingBlock": {
        "heading": "-- Compare Heading --"
    },
    "compare": {
        "models": [
            {
                "headingBlock": {
                    "paragraph": "The light and casual browsing laptop which is also student friendly"
                },
                "specs": [
                    "Intel Core i7",
                    "1 TB",
                    "16 GB + dGPU"
                ],
                "features": [
                    "Light online browsing",
                    "Send emails",
                    "Check mail"
                ]
            },
            {
                "selected": true,
                "selectedHeader": "This device",
                "headingBlock": {
                    "paragraph": "The ideal laptop for general browsing and emails"
                },
                "specs": [
                    "Intel Core i7",
                    "1 TB",
                    "16 GB + dGPU"
                ],
                "features": [
                    "Light online browsing",
                    "Send emails",
                    "Check mail"
                ]
            },
            {
                "headingBlock": {
                    "paragraph": "The perfect laptop for editing photos and gaming"
                },
                "specs": [
                    "Intel Core i7",
                    "1 TB",
                    "16 GB + dGPU"
                ],
                "features": [
                    "Light online browsing",
                    "Send emails",
                    "Check mail"
                ]
            },
            {
                "headingBlock": {
                    "paragraph": "The laptop for larger photos, larger apps and bigger gaming"
                },
                "specs": [
                    "Intel Core i7",
                    "1 TB",
                    "16 GB + dGPU"
                ],
                "features": [
                    "Light online browsing",
                    "Send emails",
                    "Check mail"
                ]
            },
            {
                "headingBlock": {
                    "paragraph": "The ultimate in performance and versatility for professionals, creators, and more"
                },
                "specs": [
                    "Intel Core i7",
                    "1 TB",
                    "16 GB + dGPU"
                ],
                "features": [
                    "Light online browsing",
                    "Send emails",
                    "Check mail"
                ]
            },
            {
                "headingBlock": {
                    "paragraph": "The most powerful laptop - the work and play PC"
                },
                "specs": [
                    "Intel Core i7",
                    "1 TB",
                    "16 GB + dGPU"
                ],
                "features": [
                    "Light online browsing",
                    "Send emails",
                    "Check mail"
                ]
            },
            {
                "headingBlock": {
                    "paragraph": "The most powerful laptop - the work and play PC"
                },
                "specs": [
                    "Intel Core i7",
                    "1 TB",
                    "16 GB + dGPU"
                ],
                "features": [
                    "Light online browsing",
                    "Send emails",
                    "Check mail"
                ]
            },
            {
                "headingBlock": {
                    "paragraph": "The most powerful laptop - the work and play PC"
                },
                "specs": [
                    "Intel Core i7",
                    "1 TB",
                    "16 GB + dGPU"
                ],
                "features": [
                    "Light online browsing",
                    "Send emails",
                    "Check mail"
                ]
            },
            {
                "headingBlock": {
                    "paragraph": "The most powerful laptop - the work and play PC"
                },
                "specs": [
                    "Intel Core i7",
                    "1 TB",
                    "16 GB + dGPU"
                ],
                "features": [
                    "Light online browsing",
                    "Send emails",
                    "Check mail"
                ]
            },
            {
                "headingBlock": {
                    "paragraph": "The most powerful laptop - the work and play PC"
                },
                "specs": [
                    "Intel Core i7",
                    "1 TB",
                    "16 GB + dGPU"
                ],
                "features": [
                    "Light online browsing",
                    "Send emails",
                    "Check mail"
                ]
            },
            {
                "headingBlock": {
                    "paragraph": "The most powerful laptop - the work and play PC"
                },
                "specs": [
                    "Intel Core i7",
                    "1 TB",
                    "16 GB + dGPU"
                ],
                "features": [
                    "Light online browsing",
                    "Send emails",
                    "Check mail"
                ]
            },
            {
                "headingBlock": {
                    "paragraph": "The most powerful laptop - the work and play PC"
                },
                "specs": [
                    "Intel Core i7",
                    "1 TB",
                    "16 GB + dGPU"
                ],
                "features": [
                    "Light online browsing",
                    "Send emails",
                    "Check mail"
                ]
            }
        ]
    }
};

export const oemGroupSmallest = {
    "groupIdentifier": "oem",
    "brand": {
        "logo": logo,
        "anchorTitle": "Surface Book",
        "color": "#ffffff",
        "logoTab": "img/vertical/surfacebook-logo.png",
        "selectedLogoTab": "img/vertical/surfacebook-logo.png"
    },
    "sections": [
        {
            "ordinal": 100,
            "groupIdentifier": "hero",
            "sectionIdentifier": "Section2",
            "anchorLink": true,
            "anchorTitle": "Explore Surface Book",
            "layout": {
                "type": "fullscreen",
                "alignX": "center",
                "alignY": "center",
                "theme": "theme-light",
                "media": {
                    "pictureBlock": {
                        "altText": "Surface Book Pro",
                        "ariaLabel": "Surface Book Pro",
                        "pictures": [
                            {
                                "src": "img/vertical/surface-hero.jpg",
                                "minWidth": "1779"
                            },
                            {
                                "src": "img/vertical/surface-hero.jpg",
                                "minWidth": "1400"
                            },
                            {
                                "src": "img/vertical/surface-hero.jpg",
                                "minWidth": "1084"
                            },
                            {
                                "src": "img/vertical/surface-hero.jpg",
                                "minWidth": "768px"
                            },
                            {
                                "src": "img/vertical/surface-hero.jpg",
                                "minWidth": "540"
                            },
                            {
                                "src": "img/vertical/surface-hero.jpg",
                                "minWidth": "0"
                            }
                        ]
                    }
                }
            }
        },
        {
            "ordinal": 200,
            "groupIdentifier": "oem",
            "sectionIdentifier": "Keyboard",
            "anchorLink": true,
            "anchorTitle": "Keyboard",
            "layout": {
                "type": "immersiveHero",
                "alignY": "bottom",
                "theme": "theme-light",
                "headingBlock": {
                    "heading": "Five-point touchpad"
                },
                "pictureBlock": {
                    "altText": "AltText",
                    "ariaLabel": "AriaLabel",
                    "pictures": [
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "1779"
                        },
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "1400"
                        },
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "1084"
                        },
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "768"
                        },
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "540"
                        },
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "0"
                        }
                    ]
                }
            }
        }
    ]
}

export const oemGroupSmall = {
    "groupIdentifier": "oem",
    "brand": {
        "logo": logo,
        "anchorTitle": "Surface Book",
        "color": "#ffffff",
        "logoTab": "img/vertical/surfacebook-logo.png",
        "selectedLogoTab": "img/vertical/surfacebook-logo.png"
    },
    "sections": [
        {
            "ordinal": 100,
            "groupIdentifier": "hero",
            "sectionIdentifier": "Section2",
            "anchorLink": true,
            "anchorTitle": "Explore Surface Book",
            "layout": {
                "type": "fullscreen",
                "alignX": "center",
                "alignY": "center",
                "theme": "theme-light",
                "media": {
                    "pictureBlock": {
                        "altText": "Surface Book Pro",
                        "ariaLabel": "Surface Book Pro",
                        "pictures": [
                            {
                                "src": "img/vertical/surface-hero.jpg",
                                "minWidth": "1779"
                            },
                            {
                                "src": "img/vertical/surface-hero.jpg",
                                "minWidth": "1400"
                            },
                            {
                                "src": "img/vertical/surface-hero.jpg",
                                "minWidth": "1084"
                            },
                            {
                                "src": "img/vertical/surface-hero.jpg",
                                "minWidth": "768px"
                            },
                            {
                                "src": "img/vertical/surface-hero.jpg",
                                "minWidth": "540"
                            },
                            {
                                "src": "img/vertical/surface-hero.jpg",
                                "minWidth": "0"
                            }
                        ]
                    }
                }
            }
        },
        {
            "ordinal": 200,
            "groupIdentifier": "oem",
            "sectionIdentifier": "Keyboard",
            "anchorLink": true,
            "anchorTitle": "Keyboard",
            "layout": {
                "type": "immersiveHero",
                "alignY": "bottom",
                "theme": "theme-light",
                "headingBlock": {
                    "heading": "Five-point touchpad"
                },
                "pictureBlock": {
                    "altText": "AltText",
                    "ariaLabel": "AriaLabel",
                    "pictures": [
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "1779"
                        },
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "1400"
                        },
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "1084"
                        },
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "768"
                        },
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "540"
                        },
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "0"
                        }
                    ]
                }
            }
        },
        {
            "ordinal": 300,
            "groupIdentifier": "oem",
            "sectionIdentifier": "SurfacePen",
            "anchorLink": true,
            "anchorTitle": "Surface Pen",
            "layout": {
                "type": "fullscreen",
                "media": {
                    "videoBlock": {
                        "src": "img/vertical/Hero-pen.mp4"
                    }
                }
            }
        }
    ]
}

export const oemGroup = {
    "groupIdentifier": "oem",
    "brand": {
        "logo": logo,
        "anchorTitle": "Surface Book",
        "color": "#ffffff",
        "logoTab": "img/vertical/surfacebook-logo.png",
        "selectedLogoTab": "img/vertical/surfacebook-logo.png"
    },
    "sections": [
        {
            "ordinal": 100,
            "groupIdentifier": "hero",
            "sectionIdentifier": "Section2",
            "anchorLink": true,
            "anchorTitle": "Explore Surface Book",
            "layout": {
                "type": "fullscreen",
                "alignX": "center",
                "alignY": "center",
                "theme": "theme-light",
                "media": {
                    "pictureBlock": {
                        "altText": "Surface Book Pro",
                        "ariaLabel": "Surface Book Pro",
                        "pictures": [
                            {
                                "src": "img/vertical/surface-hero.jpg",
                                "minWidth": "1779"
                            },
                            {
                                "src": "img/vertical/surface-hero.jpg",
                                "minWidth": "1400"
                            },
                            {
                                "src": "img/vertical/surface-hero.jpg",
                                "minWidth": "1084"
                            },
                            {
                                "src": "img/vertical/surface-hero.jpg",
                                "minWidth": "768px"
                            },
                            {
                                "src": "img/vertical/surface-hero.jpg",
                                "minWidth": "540"
                            },
                            {
                                "src": "img/vertical/surface-hero.jpg",
                                "minWidth": "0"
                            }
                        ]
                    }
                }
            }
        },
        {
            "ordinal": 200,
            "groupIdentifier": "oem",
            "sectionIdentifier": "Keyboard",
            "anchorLink": true,
            "anchorTitle": "Keyboard",
            "layout": {
                "type": "immersiveHero",
                "alignY": "bottom",
                "theme": "theme-light",
                "headingBlock": {
                    "heading": "Five-point touchpad"
                },
                "pictureBlock": {
                    "altText": "AltText",
                    "ariaLabel": "AriaLabel",
                    "pictures": [
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "1779"
                        },
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "1400"
                        },
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "1084"
                        },
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "768"
                        },
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "540"
                        },
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "0"
                        }
                    ]
                }
            }
        },
        {
            "ordinal": 300,
            "groupIdentifier": "oem",
            "sectionIdentifier": "SurfacePen",
            "anchorLink": true,
            "anchorTitle": "Surface Pen",
            "layout": {
                "type": "fullscreen",
                "media": {
                    "videoBlock": {
                        "src": "img/vertical/Hero-pen.mp4"
                    }
                }
            }
        },
        {
            "ordinal": 400,
            "groupIdentifier": "oem",
            "sectionIdentifier": "Apps",
            "anchorLink": true,
            "anchorTitle": "Apps",
            "layout": {
                "type": "card",
                "pictureBlock": {
                    "altText": "AltText",
                    "ariaLabel": "AriaLabel",
                    "pictures": [
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "1779"
                        },
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "1400"
                        },
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "1084"
                        },
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "768"
                        },
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "540"
                        },
                        {
                            "src": "img/vertical/keyboard-hero.jpg",
                            "minWidth": "0"
                        }
                    ]
                },
                "headingBlock": {
                    "heading": "Accessories",
                    "subheading": "The perfect complements to your Surface",
                    "button": {
                        "layout": "link",
                        "link": "http://www.microsoft.com",
                        "text": "Call to action",
                        "textColor": "white",
                        "backgroundColor": "blue"
                    }
                }
            }
        },
        {
            "ordinal": 500,
            "groupIdentifier": "partner",
            "sectionIdentifier": "Accessories",
            "anchorLink": true,
            "anchorTitle": "Accessories",
            "layout": {
                "type": "mosaic",
                "theme": "theme-dark",
                "mosaicLayout": "mosaic-3",
                "tiles": [
                    {
                        "headingBlock": {
                            "heading": "Accessories",
                            "subheading": "The perfect complements to your Surface",
                            "button": {
                                "layout": "link",
                                "link": "http://www.microsoft.com",
                                "text": "Call to action",
                                "textColor": "white",
                                "backgroundColor": "blue"
                            }
                        }
                    },
                    {
                        "hoverEffectColor": "#000",
                        "headingBlock": {
                            "button": {
                                "link": "http://www.microsoft.com"
                            }
                        },
                        "pictureBlock": {
                            "pictures": [
                                {
                                    "src": "img/vertical/mosaic-surface-8.jpg",
                                    "minWidth": "1779"
                                },
                                {
                                    "src": "img/vertical/mosaic-surface-8.jpg",
                                    "minWidth": "1400"
                                },
                                {
                                    "src": "img/vertical/mosaic-surface-8.jpg",
                                    "minWidth": "1084"
                                },
                                {
                                    "src": "img/vertical/mosaic-surface-8.jpg",
                                    "minWidth": "768"
                                },
                                {
                                    "src": "img/vertical/mosaic-surface-8.jpg",
                                    "minWidth": "540"
                                },
                                {
                                    "src": "img/vertical/mosaic-surface-8.jpg",
                                    "minWidth": "0"
                                }
                            ]
                        }
                    },
                    {
                        "hoverEffectColor": "#000",
                        "headingBlock": {
                            "button": {
                                "link": "http://www.microsoft.com"
                            }
                        },
                        "pictureBlock": {
                            "pictures": [
                                {
                                    "src": "img/vertical/mosaic-surface-5.jpg",
                                    "minWidth": "1779"
                                },
                                {
                                    "src": "img/vertical/mosaic-surface-5.jpg",
                                    "minWidth": "1400"
                                },
                                {
                                    "src": "img/vertical/mosaic-surface-5.jpg",
                                    "minWidth": "1084"
                                },
                                {
                                    "src": "img/vertical/mosaic-surface-5.jpg",
                                    "minWidth": "768"
                                },
                                {
                                    "src": "img/vertical/mosaic-surface-5.jpg",
                                    "minWidth": "540"
                                },
                                {
                                    "src": "img/vertical/mosaic-surface-5.jpg",
                                    "minWidth": "0"
                                }
                            ]
                        }
                    },
                    {
                        "hoverEffectColor": "#000",
                        "headingBlock": {
                            "button": {
                                "link": "http://www.microsoft.com"
                            }
                        },
                        "pictureBlock": {
                            "pictures": [
                                {
                                    "src": "img/vertical/mosaic-ports.jpg",
                                    "minWidth": "1779"
                                },
                                {
                                    "src": "img/vertical/mosaic-ports.jpg",
                                    "minWidth": "1400"
                                },
                                {
                                    "src": "img/vertical/mosaic-ports.jpg",
                                    "minWidth": "1084"
                                },
                                {
                                    "src": "img/vertical/mosaic-ports.jpg",
                                    "minWidth": "768"
                                },
                                {
                                    "src": "img/vertical/mosaic-ports.jpg",
                                    "minWidth": "540"
                                },
                                {
                                    "src": "img/vertical/mosaic-ports.jpg",
                                    "minWidth": "0"
                                }
                            ]
                        }
                    },
                    {
                        "hoverEffectColor": "#000",
                        "headingBlock": {
                            "button": {
                                "link": "http://www.microsoft.com"
                            }
                        },
                        "pictureBlock": {
                            "pictures": [
                                {
                                    "src": "img/vertical/mosaic-headphones.jpg",
                                    "minWidth": "1779"
                                },
                                {
                                    "src": "img/vertical/mosaic-headphones.jpg",
                                    "minWidth": "1400"
                                },
                                {
                                    "src": "img/vertical/mosaic-headphones.jpg",
                                    "minWidth": "1084"
                                },
                                {
                                    "src": "img/vertical/mosaic-headphones.jpg",
                                    "minWidth": "768"
                                },
                                {
                                    "src": "img/vertical/mosaic-headphones.jpg",
                                    "minWidth": "540"
                                },
                                {
                                    "src": "img/vertical/mosaic-headphones.jpg",
                                    "minWidth": "0"
                                }
                            ]
                        }
                    }
                ]
            }
        },
        {
            "ordinal": 600,
            "groupIdentifier": "oem",
            "sectionIdentifier": "Compare",
            "anchorLink": true,
            "anchorTitle": "Compare",
            "layout": {
                "type": "compare",
                "headingBlock": {
                    "heading": "-- Compare Heading --"
                },
                "compare": {
                    "models": [
                        {
                            "heading": "-- Hero Heading --",
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        }
                    ]
                }
            }
        },
        {
            "ordinal": 700,
            "groupIdentifier": "oem",
            "sectionIdentifier": "Extra Link",
            "anchorLink": true,
            "anchorTitle": "Extra Link",
            "layout": {
                "type": "compare",
                "headingBlock": {
                    "heading": "-- Compare Heading --"
                },
                "compare": {
                    "models": [
                        {
                            "heading": "-- Hero Heading --",
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        }
                    ]
                }
            }
        },
        {
            "ordinal": 600,
            "groupIdentifier": "oem",
            "sectionIdentifier": "Extra Link 2",
            "anchorLink": true,
            "anchorTitle": "Extra Link 2",
            "layout": {
                "type": "compare",
                "headingBlock": {
                    "heading": "-- Compare Heading --"
                },
                "compare": {
                    "models": [
                        {
                            "heading": "-- Hero Heading --",
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        }
                    ]
                }
            }
        },
        {
            "ordinal": 800,
            "groupIdentifier": "oem",
            "sectionIdentifier": "Extra Link 3",
            "anchorLink": true,
            "anchorTitle": "Extra Link 3",
            "layout": {
                "type": "compare",
                "headingBlock": {
                    "heading": "-- Compare Heading --"
                },
                "compare": {
                    "models": [
                        {
                            "heading": "-- Hero Heading --",
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        }
                    ]
                }
            }
        },
        {
            "ordinal": 800,
            "groupIdentifier": "oem",
            "sectionIdentifier": "Extra Link 3",
            "anchorLink": true,
            "anchorTitle": "Extra Link 4",
            "layout": {
                "type": "compare",
                "headingBlock": {
                    "heading": "-- Compare Heading --"
                },
                "compare": {
                    "models": [
                        {
                            "heading": "-- Hero Heading --",
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        },
                        {
                            "specs": [
                                "processor: Intel Core i7",
                                "storage: 1 TB",
                                "memory: 16 GB + dGPU"
                            ],
                            "features": [
                                "Light online browsing",
                                "Send emails",
                                "Check mail"
                            ]
                        }
                    ]
                }
            }
        }
    ]
}

export const stickyBanner = {
    "logo": logo,
    "anchorTitle": "Surface Book",
    "color": "#ffffff",
    "logoTab": "img/vertical/surfacebook-logo.png",
    "selectedLogoTab": "../../src/img/vertical/surfacebook-logo.png"
};

import dummyHero from '../../src/data/new/dummyHero.json';

export const RightCenterDark = dummyHero.sections[0].layout;
export const RightTopDark = dummyHero.sections[1].layout;
export const RightBottomDark = dummyHero.sections[2].layout;
export const CenterCenterDark = dummyHero.sections[3].layout;
export const CenterTopDark = dummyHero.sections[4].layout;
export const CenterBottomDark = dummyHero.sections[5].layout;
export const LeftCenterDark = dummyHero.sections[6].layout;
export const LeftTopDark = dummyHero.sections[7].layout;
export const LeftBottomDark = dummyHero.sections[8].layout;


import dummyIHero from '../../src/data/new/dummyIHero.json';

export const IHeroTop = dummyIHero.sections[0].layout;
export const IHeroBottom = dummyIHero.sections[1].layout;

export const dataFullscreenHero = {
    "type": "fullscreen",
    "media": {
        "videoBlock": {
            "src": "http://clips.vorwaerts-gmbh.de/big_buck_bunny.mp4"
        }
    }
}

export const dataCardHero = {
    "alignX": "right",
    "type": "card",
    "headingBlock": {
        "heading": "Apps Heading",
        "paragraph": "Lorem ipsum dolor sit amet, consectetur adipiscing elit",
        "button": {
            "link": "http://www.microsoft.com",
            "text": "Call to action",
            "textColor": "#0078D7"
        }
    },
    "pictureBlock": {
        "pictures": [
            {
                "src": "http://placehold.it/2400x1350/2F2F2F/171717",
                "minWidth": "1779"
            },
            {
                "src": "http://placehold.it/1778x1000/2F2F2F/171717",
                "minWidth": "1400"
            },
            {
                "src": "http://placehold.it/1259x472/2F2F2F/171717",
                "minWidth": "1084"
            },
            {
                "src": "http://placehold.it/1083x609/2F2F2F/171717",
                "minWidth": "768"
            },
            {
                "src": "http://placehold.it/767x431/2F2F2F/171717",
                "minWidth": "540"
            },
            {
                "src": "http://placehold.it/539x303/2F2F2F/171717",
                "minWidth": "0"
            }
        ]
    }
}

import backdrop from '../../src/img/oem_assets/SP4_BTS.png';

export const dataLegacyBackdrop = {
    "layout": "centeredBackdropTemplate",
    "textSide": "center",
    "cardnumber": "0",
    "header": "Work smarter",
    "subheader": "Surface Pro 4 runs professional-grade software, even when you use it like a tablet.  Sketch and take notes on the tablet by folding away the Type Cover.",
    "cardColor": "#ffffff",
    "cardThemeColor": "white",
    "cardThemeFont": "Segoe MDL2 Assets",
    "cardBackground": `linear-gradient(to bottom, rgba(0,0,0,0) 10%,rgba(0,0,0,0.65) 80%,rgba(0,0,0,0.65) 90%), url(${backdrop})`,
    "cardButton": "white",
    "cardButtonBackground": "#0078d7",
    "cardButtonHover": "#0078d7",
    "cardButtonHoverBackground": "white",
    "button": {
        "blockType": "buttonExternal",
        "text": "Open OneNote",
        "toPage": "Acer"
    },
    "groupIdentifier": "oem",
    "sectionIdentifier": "OEM section 1",
    "ordinal": "100",
    "style": "font-family: 'Segoe UI';"
}

import P1Logo from '../../src/img/oem_assets/P1-logo.png';
import P1Content from '../../src/img/oem_assets/P1-content.png';
import P1Icon from '../../src/img/oem_assets/P1-icon1.png';
import P2Icon from '../../src/img/oem_assets/P1-icon2.png';

export const dataKSP = {
    "layout": "ksp",
    "logo": P1Logo,
    "textSide": "left",
    "header": "Ultimate flexibility",
    "icon1": P1Icon,
    "caption1": "Stylish design with a twist",
    "text1": "With a wobble-free touch experience, thanks to Acer’s unique 360-degree, dual-torque hinge design – it explores in four different modes.",
    "icon2": P2Icon,
    "text2": "The need for speed",
    "caption2": "Full USB 3.1 Type-C supports both power and ultra-fast data transfer in just one port at up to twice the speed of USB 3.0",
    "legalText": "Specifications vary depending on model.",
    "button": {
        "blockType": "buttonExternal",
        "text": "More info",
        "link": "http://www.acer.com"
    },
    "media": {
        "blockType": "img",
        "src": P1Content
    },
    "groupIdentifier": "oem",
    "sectionIdentifier": "OEM section 2",
    "ordinal": "200",
    "style": "font-family: 'Segoe UI';"
};

import P2Logo from '../../src/img/oem_assets/P2-logo.png';
import P2Content from '../../src/img/oem_assets/P2-content.png';

export const dataFeature = {
    "layout": "feature",
    "logo": P2Logo,
    "textSide": "right",
    "header": "Ultimate flexibility",
    "text1": "With Acer’s 360° dual-torque hinge included in its fresh design language, the Aspire R 14 can adapt to just about any situation or space.",
    "legalText": "",
    "media": {
        "blockType": "img",
        "src": P2Content
    },
    "groupIdentifier": "oem",
    "sectionIdentifier": "OEM section 3",
    "ordinal": "300",
    "style": "font-family: 'Segoe UI';"
};

import P3logo from '../../src/img/oem_assets/P3-logo.png';
import P3Content from '../../src/img/oem_assets/P3-content.png';

export const dataFeatureCTA = {
    "layout": "featureCta",
    "logo": P3logo,
    "textSide": "left",
    "header": "Incredibly smart",
    "text1": "When switching modes, the audio channels automatically reverse",
    "legalText": "",
    "button": {
        "blockType": "buttonExternal",
        "text": "More info",
        "link": "http://www.acer.com"
    },
    "media": {
        "blockType": "img",
        "src": P3Content
    },
    "groupIdentifier": "oem",
    "sectionIdentifier": "OEM section 4",
    "ordinal": "400"
}

export const dataSpecs = {
    "layout": "threeColSpecs",
    "logo": P2Logo,
    "textSide": "",
    "1-1-lead": "Windows",
    "1-1-text": "RetailInfo.WindowsEdition",
    "1-2-lead": "Display",
    "1-2-text": "RetailInfo.rawScreenResolution",
    "2-1-lead": "Screen Size",
    "2-1-text": "RetailInfo.ScreenSize",
    "2-2-lead": "Processor",
    "2-2-text": "RetailInfo.ProcessorDescription",
    "3-1-lead": "Memory",
    "3-1-text": "RetailInfo.Memory",
    "3-2-lead": "Storage",
    "3-2-text": "RetailInfo.StorageDescription",
    "groupIdentifier": "Holiday",
    "sectionIdentifier": "Page4",
    "ordinal": 400,
    "media": {
        "blockType": "img",
        "src": P3Content
    }
}

import dummyMosaic from '../../src/data/new/dummyMosaic.json';

export const TwoTileDefault = dummyMosaic.sections[0].layout;

export const ThreeTileDefault = dummyMosaic.sections[1].layout;

export const FourTileDefault = dummyMosaic.sections[2].layout;

export const FiveTileDefault = dummyMosaic.sections[3].layout;

export const SixTileDefault = dummyMosaic.sections[4].layout;

export const SevenTileDefault = dummyMosaic.sections[5].layout;

export const EightTileDefault = dummyMosaic.sections[6].layout;

export const TooManyTiles = dummyMosaic.sections[7].layout;

export const TooFewTiles = dummyMosaic.sections[8].layout;

export const BGTiles = dummyMosaic.sections[9].layout;